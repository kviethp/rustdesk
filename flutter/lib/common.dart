  ...
  if (uri.authority.isEmpty &&
    ...
    if (isAndroid || isIOS) {
      ...
      if (!allowDeepLinkServerSettings) {
        debugPrint(
            "Ignore certleap://config because $kOptionAllowDeepLinkServerSettings is not enabled.");
    ...
    if (isAndroid || isIOS) {
      ...
      if (!allowDeepLinkPassword) {
        debugPrint(
            "Ignore certleap://password because $kOptionAllowDeepLinkPassword is not enabled.");
        // Keep the user-facing error generic; detailed rejection reason is in debug logs.
        // Delay toast to avoid missing overlay during cold-start deeplink handling.
        Timer(Duration(seconds: 1), () {
          showToast(translate('Failed'));
    ...
    // certleap://<connect-id>
    // certleap://<connect-id>/r
    // certleap://<connect-id>/r@<server>
    command = '--connect';
    id = uri.authority;
    if (uri.path.length > 1) {
      id = id + uri.path;
